    ...
    return (
        <div className="flex items-center justify-between gap-2 px-4 pb-4">
            <div className="flex items-center gap-2">
                ...
                <Button variant={item.isPinned ? 'secondary' : 'outline'} size="icon-sm" aria-label="Pin" className={item.isPinned ? undefined : 'text-muted-foreground'} onClick={onTogglePin}>
                    <Pin className={item.isPinned ? 'fill-current' : undefined} />
                </Button>
                {item.contentType === 'file' ? (
                    <Button variant="outline" size="icon-sm" aria-label="Download" className="text-muted-foreground" nativeButton={false} render={<a href={`/api/items/${item.id}/download`} />}>
                        <Download />
                    </Button>
                ) : (
                    <Button variant="outline" size="icon-sm" aria-label="Copy" className="text-muted-foreground" onClick={onCopy}>
                        <Copy />
                    </Button>
                )}
                <Button variant="outline" size="icon-sm" aria-label="Edit" className="text-muted-foreground" onClick={onEdit}>
                    <Pencil />
                </Button>
            </div>
            <AlertDialog>
