...
interface OptimizePromptTriggerProps {
  isPro: boolean;
  loading: boolean;
  onClick: () => void;
}

export function OptimizePromptTrigger({ isPro, loading, onClick }: OptimizePromptTriggerProps) {
  if (!isPro) {
    return (
      <Tooltip>
        <TooltipTrigger
          render={
            <Button
              ...
              variant="ghost"
              size="sm"
              aria-label="Optimize (requires Pro)"
              aria-disabled="true"
