...
`prisma/seed.ts` sets `contentType` per item as `data.url ? "url" : "text"` and never seeds `file`/`image` items, so those two types have zero _seeded_ rows — but the upload flow itself (`feature/file-image-upload`) is implemented, and users can create real `file`/`image` items via upload.
...
- **`CollectionCard`** (`src/components/collections/CollectionCard.tsx`): since a collection holds mixed types, it shows a single dot colored by the _dominant_ item type in the collection, plus a small row of icons (one per distinct type present, deduped) — not per-item colors.
